...
ThreadRec::ThreadRec(int argc, char** argv) {
    s = -1;
    running = 1;
    filter = NULL;
    filter_count = 0;
...
int ThreadRec::add_filter(u_int32_t id, u_int32_t mask) {
...
void ThreadRec::print_usage(char *prg) {
    fprintf(
            stderr,
...
int ThreadRec::MyRecieve(int argc, char **argv) {
    ...
    struct option long_options[] = { { "help", no_argument, 0, 'h' }, {
            ...
            { "filter", required_argument, 0, FILTER_OPTION }, { "version",
                    no_argument, 0, VERSION_OPTION }, { 0, 0, 0, 0 }, };
...
//	char *ps[5] = {"candump","can1"," "};
    ...
    while (running) {
        if ((nbytes = read(s, &frame, sizeof(struct can_frame))) < 0) {
            ...
            n = 0;
          /*  if(frame.can_id == 16)         //桢ID  16
                 FlagCan0 = 1;          //重要,代表周期开始了
            if(frame.can_id == 291)     //如果这个周期内收到291的信号，标志位置1，继续用can0发送
                FlagCan0 = 1;
            if(frame.can_id == 1)
                FlagCan0 = 1;*/
            emit mesRecv(buf);
